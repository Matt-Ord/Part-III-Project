...
class TestHamiltonian(unittest.TestCase):
    def test_save_no_result(self):
        solver = RotatingWaveLindbaldSolver(
            times=np.linspace(0, 10 ** (-9), 10), temperature=150
        )
        solver.save_to_file("test2.npz")

        solver2 = RotatingWaveLindbaldSolver.load_from_file("test2.npz")
    ...
    def test_save_result(self):
        ...
        solver2 = RotatingWaveLindbaldSolver.load_from_file("test.npz")
